...
Cycle optimizedApplyTransposition(Cycle pi, Move move) {
    ...
    int indexes[3];
    Cycle result_cycle;
    result_cycle.symbols = malloc(pi.sizeSymbols * sizeof(int));
    result_cycle.symbolsIndexes = malloc(pi.sizeSymbols * sizeof(int));
    ...
    int result_symbols [pi.sizeSymbols];
    int result_index [pi.sizeSymbols];
...
Move * generateAll0And2Moves(Permutation spi, Cycle pi)
{
    ...
    Move * moves;
    Cycle cycle_index[pi.sizeSymbols];
    Cycle moveCycle;
    int moves_size = 0;
    int is_2Move = 1;
    int a;
    ...
    Permutation * permutation = malloc(2*sizeof(Permutation));
    permutation[0].sizeMulticycle = spi.sizeMulticycle;
    permutation[0].multicycle = malloc((spi.sizeMulticycle)*sizeof(Cycle));
                    ...
                    for(int k = j + 1; k < pi.sizeSymbols; k++){
                        ...
                        if(cycle_index[pi.symbols[k]].sizeSymbols > 1){
                            ...
                            if(is_2Move == 1){
                                ...
                                int symbolsIndexes[spi.maxSymbol+1];
                                ...
                                minMax(moveCycle, limits);
                                moveCycle.minSymbol = limits[0];
                                moveCycle.maxSymbol = limits[1];
                                ...
                                if(delta >= 0){
                                    if(moves_size == 0){
                                        ...
                                        moves = realloc(moves, ++moves_size * sizeof(Move));
                                    }
                                    moves[moves_size-1].move = delta;
                                    moves[moves_size-1].cycle.symbols = malloc(moveCycle.sizeSymbols * sizeof(int));
                                    moves[moves_size-1].cycle.symbolsIndexes = malloc((spi.maxSymbol+1)*sizeof(int));
        ...
        if (moves[i].move == 2) {
            ...
            while(secondMoves[j].cycle.sizeSymbols > 0) {
                if (secondMoves[j].move == 2) {
                    Cycle * result = malloc(2*sizeof(Cycle));
